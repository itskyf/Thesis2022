...
def get_git_commit_number():
    ...
    cmd_out = subprocess.run(["git", "rev-parse", "HEAD"], check=True, stdout=subprocess.PIPE)
...
def make_cuda_ext(name, module, sources):
    cuda_ext = CUDAExtension(
        name=f"{module}.{name}", sources=[os.path.join(*module.split("."), src) for src in sources]
    )
    return cuda_ext


def write_version_to_file(pkg_version, target_file):
    with open(target_file, "w", encoding="utf-8") as ver_file:
        print(f'__version__ = "{pkg_version}"', file=ver_file)
