...
def main():
    ...
    if args.launcher == "none":
        ...
        dist_init_fn = getattr(common_utils, f"init_dist_{args.launcher}")
        total_gpus, config.LOCAL_RANK = dist_init_fn(args.tcp_port, args.local_rank, backend="nccl")
        dist_train = True
    ...
    args.epochs = config.OPTIMIZATION.NUM_EPOCHS if args.epochs is None else args.epochs

    if args.fix_random_seed:
        common_utils.set_random_seed(666)

    output_dir = config.ROOT_DIR / "output" / config.EXP_GROUP_PATH / config.TAG / args.extra_tag
    ...
    tb_log = (
        SummaryWriter(log_dir=str(output_dir / "tensorboard")) if config.LOCAL_RANK == 0 else None
    )
    ...
    model = build_network(
        model_cfg=config.MODEL, num_class=len(config.CLASS_NAMES), dataset=train_set
    )
    if args.sync_bn:
    ...
    if dist_train:
        model = nn.parallel.DistributedDataParallel(
            model, device_ids=[config.LOCAL_RANK % torch.cuda.device_count()]
        )
    logger.info(model)
    ...
    logger.info(
        "**********************End training %s/%s(%s)**********************\n\n\n"
        % (config.EXP_GROUP_PATH, config.TAG, args.extra_tag)
    )
