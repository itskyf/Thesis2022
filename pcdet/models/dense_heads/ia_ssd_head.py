...
class IASSD_Head(PointHeadTemplate):
    ...
    def __init__(
        ...
        self.cls_center_layers = self.make_fc_layers(
            fc_cfg=self.model_cfg.CLS_FC, input_channels=detector_dim, output_channels=num_class
        )
        ...
        self.box_iou3d_layers = (
            self.make_fc_layers(
                fc_cfg=self.model_cfg.IOU_FC, input_channels=detector_dim, output_channels=1
            )
            if self.model_cfg.get("IOU_FC", None) is not None
            else None
        )
    ...
    def build_losses(self, losses_cfg):
        ...
        if losses_cfg.get("LOSS_INS", None) is not None:
            ...
            elif losses_cfg.LOSS_INS.startswith("PolyWeightedCrossEntropy"):
                self.add_module("ins_loss_func", loss_utils.PolyWeightedCrossEntropyLoss(epsilon=1))
            else:
                raise NotImplementedError
    ...
    def assign_stack_targets_IASSD(
        ...
        for k in range(batch_size):
            ...
            if use_query_assign:  ##
                ...
                query_idxs_of_pts = (
                    roiaware_pool3d_utils.points_in_ball_query_gpu(
            ...
            point_cls_labels_single[fg_flag] = (
                1 if self.num_class == 1 or binary_label else gt_box_of_fg_points[:, -1].long()
            )
    ...
    def assign_targets(self, input_dict):
        ...
        targets_dict_center["center_gt_box_of_fg_points"] = center_targets_dict[
            "gt_box_of_fg_points"
        ]
        ...
        if extra_method is not None and extra_method.NAME == "extend_gt":
            ...
            targets_dict = self.assign_stack_targets_IASSD(
                ...
                ret_box_labels=True,
                use_ex_gt_assign=True,
                fg_pc_ignore=extra_method.FG_PC_IGNORE,
            )
            targets_dict_center["center_origin_gt_box_of_fg_points"] = targets_dict[
                "gt_box_of_fg_points"
            ]
            ...
            targets_dict = self.assign_stack_targets_IASSD(
                ...
                use_ball_constraint=False,
                ret_box_labels=False,
                use_query_assign=True,
                central_radii=extra_method.RADII,
                fg_pc_ignore=extra_method.FG_PC_IGNORE,
            )
            targets_dict_center["center_origin_gt_box_of_fg_points"] = targets_dict[
                "gt_box_of_fg_points"
    ...
    def get_loss(self, tb_dict=None):
        ...
        # vote loss
        if (
            self.model_cfg.TARGET_CONFIG.get("ASSIGN_METHOD") is not None
            and self.model_cfg.TARGET_CONFIG.ASSIGN_METHOD.get("ASSIGN_TYPE") == "centers_origin"
        ):
        ...
        if self.model_cfg.LOSS_CONFIG.get("LOSS_INS", None) is not None:
            assert ("sa_ins_preds" in self.forward_ret_dict) and (
    ...
    def get_contextual_vote_loss(self, tb_dict=None):
        ...
        for i in self.forward_ret_dict["center_origin_cls_labels"].unique():
            ...
            center_box_labels = self.forward_ret_dict["center_origin_gt_box_of_fg_points"][:, 0:3][
            ...
            simple_center_origin_loss_box = functional.smooth_l1_loss(
        ...
        center_origin_loss_box = (
            center_origin_loss_box * self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS.get("vote_weight")
        )
        if tb_dict is None:
            tb_dict = {}
    ...
    def get_contextual_vote_loss_ver1(self, tb_dict=None):
        ...
        for cur_id in batch_idx.unique():
            ...
            for ins_idx in box_idxs_of_pts[batch_mask].unique():
                ...
                ins_vote_loss.append(
                    functional.smooth_l1_loss(
                        centers_pred[batch_mask][ins_mask, 1:4],
                        center_box_labels[batch_mask][ins_mask, 0:3],
                        reduction="sum",
                    ).unsqueeze(-1)
                )
        ...
        for cur_id in batch_idx.unique():
            ...
            for ins_idx in box_idxs_of_pts[batch_mask].unique():
                if ins_idx < 0:
                    continue
                ins_mask = (
                    box_idxs_of_pts[batch_mask] == ins_idx
                )  # box_idxs_of_pts[batch_mask][ins_mask]
                ...
                ins_mean_vote_loss.append(
                    functional.smooth_l1_loss(
                        centers_pred[batch_mask][ins_mask, 1:4],
                        centers_pred[batch_mask][ins_mask, 1:4]
                        .mean(dim=0)
                        .repeat(centers_pred[batch_mask][ins_mask, 1:4].shape[0], 1),
                        reduction="sum",
                    ).unsqueeze(-1)
                )
        ins_num = torch.cat(ins_num, dim=-1).float()
    ...
    def get_vote_loss_loss(self, tb_dict=None):
        ...
        vote_loss = functional.smooth_l1_loss(centers_pred, center_box_labels, reduction="mean")
        if tb_dict is None:
            tb_dict = {}
    ...
    def get_sa_ins_layer_loss(self, tb_dict=None):
        ...
        for i in range(len(sa_ins_labels)):  # valid when i =1, 2
            ...
            one_hot_targets.scatter_(
                -1,
                (point_cls_labels * (point_cls_labels >= 0).long()).unsqueeze(dim=-1).long(),
                1.0,
            )
            one_hot_targets = one_hot_targets[..., 1:]
            ...
            point_loss_ins = (
                self.ins_loss_func(point_cls_preds, one_hot_targets, weights=cls_weights)
                .mean(dim=-1)
                .sum()
            )
            loss_weights_dict = self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS
            point_loss_ins = (
                point_loss_ins
                * loss_weights_dict.get("ins_aware_weight", [1] * len(sa_ins_labels))[i]
            )
    ...
    def generate_center_ness_mask(self):
        ...
        offset_xyz_canical = common_utils.rotate_points_along_z(
            offset_xyz.unsqueeze(dim=1), -gt_boxes[:, 6]
        ).squeeze(dim=1)
    ...
    def get_iou3d_layer_loss(self, tb_dict=None):
        ...
        if loss_iou3d != None:
            ...
            tb_dict.update({'iou3d_loss_reg': 0.0})
        return loss_iou3d, tb_dict
    ...
    def forward(self, batch_dict):
        ...
        if (
            not self.training
            or self.predict_boxes_when_training
            or self.model_cfg.LOSS_CONFIG.CORNER_LOSS_REGULARIZATION
            or self.model_cfg.LOSS_CONFIG.CENTERNESS_REGULARIZATION
            or self.model_cfg.LOSS_CONFIG.IOU3D_REGULARIZATION
        ):

            point_cls_preds, poinimport torch
from torch import nn
from torch.nn import functional
...
class IASSD_Head(PointHeadTemplate):
    ...
    def forward(self, batch_dict):
        ...
        return batch_dictt_box_preds = self.generate_predicted_boxes(
                points=center_coords[:, 1:4],
                point_cls_preds=center_cls_preds,
                point_box_preds=center_box_preds,
            )
