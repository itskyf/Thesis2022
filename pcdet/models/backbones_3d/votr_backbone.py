...
class SparseTensor:
    ...
    def dense(self, channels_first=True):
        ...
        output_shape = [self.batch_size] + list(reverse_spatial_shape) + [self.features.shape[1]]
...
class Attention3d(nn.Module):
    def __init__(
        ...
        self.output_layer = nn.Sequential(
            nn.Linear(input_channels, output_channels), nn.BatchNorm1d(output_channels), nn.ReLU()
        )
...
class SparseAttention3d(Attention3d):
    ...
    def create_gather_dict(self, attention_modes, map_table, voxel_indices, spatial_shape):
        ...
        for attention_mode in attention_modes:
            if attention_mode.NAME == "LocalAttention":
                ...
                _gather_indices = votr_utils.sparse_local_attention_hash_indices(
                    spatial_shape, attend_size, attend_range, self.strides, map_table, voxel_indices
                )
            elif attention_mode.NAME == "StridedAttention":
                attend_size = attention_mode.SIZE
                range_spec = attention_mode.RANGE_SPEC
                _gather_indices = votr_utils.sparse_strided_attention_hash_indices(
                    spatial_shape, attend_size, range_spec, self.strides, map_table, voxel_indices
                )
            else:
                raise NotImplementedError
    ...
    def forward(self, sp_tensor):
        ...
        key_features = votr_utils.grouping_operation(
            voxel_features, v_bs_cnt, key_indices, k_bs_cnt
        )
        voxel_coords = self.with_coords(
            sp_tensor.indices, sp_tensor.point_cloud_range, sp_tensor.voxel_size
        )
...
class SubMAttention3d(Attention3d):
    ...
    def forward(self, sp_tensor):
        if not sp_tensor.gather_dict:
            sp_tensor.gather_dict = self.create_gather_dict(
        ...
        if self.use_pos_emb:
            ...
            key_coords = votr_utils.grouping_operation(
                voxel_coords, v_bs_cnt, key_indices, k_bs_cnt
            )
            if self.use_relative_coords:
...
class VoxelTransformer(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, voxel_size, point_cloud_range):
        ...
        self.input_transform = nn.Sequential(
            nn.Linear(input_channels, 16), nn.BatchNorm1d(16), nn.ReLU()
        )
    ...
    def forward(self, batch_dict):
        ...
        batch_dict.update({"encoded_spconv_tensor": sp_tensor, "encoded_spconv_tensor_stride": 8})
        return batch_dict
...
class SparseAttention3dv2(Attention3d):
    def __init__(
        ...
        super(SparseAttention3dv2, self).__init__(
            input_channels, output_channels, ff_channels, dropout, num_heads, attention_modes
        )
        ...
        for batch_idx in range(batch_size):
            ...
            if self.global_mode.TYPE == "TopK":
                ...
                with torch.no_grad():
                    ...
                    global_features_single = (
                        torch.matmul(pca_features, v).transpose(0, 1).contiguous()
                    )  # (K, C)
            ...
            global_features_single = global_features_single.unsqueeze(0).repeat(
                num_voxels_single, 1, 1
            )  # (N, C, K)
            global_coords_single = global_coords_single.unsqueeze(0).repeat(
                num_voxels_single, 1, 1
            )  # (N, 3, K)
    ...
    def forward(self, sp_tensor):
        ...
        if self.global_mode:
            global_key_features, global_key_coords = self.global_attention(
            ...
            global_key_mask = torch.zeros(
                (key_mask.shape[0], self.global_mode.SIZE), dtype=key_mask.dtype
            ).to(key_mask.device)
