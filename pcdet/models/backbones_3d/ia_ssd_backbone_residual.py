...
from ...ops.pointnet2.pointnet2_batch import pointnet2_modules_residual


class IASSD_Backbone_Residual(nn.Module):
    """Backbone for IA-SSD"""
    ...
    def __init__(self, model_cfg, num_class, input_channels, **kwargs):
        ...
        self.residual_blocks_2d = sa_config.get('RESIDUAL_BLOCKS_2D', None)
        self.aggregation_mlps = sa_config.get("AGGREGATION_MLPS", None)
        self.residual_blocks = sa_config.get('RESIDUAL_BLOCKS', None)
            ...
            if self.layer_types[k] == "SALayer":
                ...
                self.sa_modules.append(
                    pointnet2_modules.PointnetSAModuleMSG_WithSampling(
                        ...
                        residual_blocks=self.residual_blocks[k],
