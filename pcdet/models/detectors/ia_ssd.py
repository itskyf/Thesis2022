    ...
    def post_processing_iou(self, batch_dict):
        ...
        for index in range(batch_size):
            if batch_dict.get("batch_index", None) is not None:
                ...
                assert batch_dict["box_iou3d_preds"].dim() == 2
                batch_mask = batch_dict["batch_index"] == index
            else:
                assert batch_dict["batch_box_preds"].dim() == 3
                assert batch_dict["box_iou3d_preds"].dim()
                batch_mask = index
            ...
            alpha = post_process_cfg.NMS_CONFIG.get('ALPHA', 0.75)
            if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                ...
                for cur_cls_preds, cur_label_mapping in zip(cls_preds, multihead_label_mapping):
                    ...
                    (
                        ...
                        cls_scores=torch.pow(cur_cls_preds, 1 - alpha) + torch.pow(cur_box_iou, alpha),
                ...
                selected, selected_scores = model_nms_utils.class_agnostic_nms(
                    box_scores= torch.pow(cls_preds, 1 - alpha) + torch.pow(box_iou3d_preds, alpha),
                    box_preds=box_preds,
                    nms_config=post_process_cfg.NMS_CONFIG,
                    score_thresh=post_process_cfg.SCORE_THRESH,
                )
