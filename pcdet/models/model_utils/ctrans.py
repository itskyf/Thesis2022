...
class Transformer(nn.Module):
    ...
    def forward(self, src, query_embed, pos_embed):
        ...
        hs = self.decoder(
            tgt, memory, memory_key_padding_mask=None, pos=pos_embed, query_pos=query_embed
        )
...
def attention(query, key, value):
    dim = query.shape[1]
    scores_1 = torch.einsum("bdhn,bdhm->bhnm", query, key) / dim**0.5
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(
        ...
        tgt2 = self.multihead_attn(
            query=self.with_pos_embed(tgt, query_pos).permute(1, 2, 0),
