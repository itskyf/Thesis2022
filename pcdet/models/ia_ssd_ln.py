...
from .backbones_3d.ia_ssd import IASSDEncLN, VoteLayerLN
from .backbones_3d.pfe import PointNetSAMSGLN
from .dense_heads.ia_ssd_ln import IASSDHeadLN
from .post_process import post_processing


class IASSDLayerNorm(nn.Module):
    def __init__(self, model_cfg, n_class: int):
        super().__init__()
        self.encoder = IASSDEncLN(n_class, **model_cfg.enc_cfg)
        self.vote_layer = VoteLayerLN(n_class, **model_cfg.vote_cfg)
        self.ctr_agg_layer = PointNetSAMSGLN(**model_cfg.ctr_agg_cfg)
        self.point_head = IASSDHeadLN(n_class, **model_cfg.head_cfg)
