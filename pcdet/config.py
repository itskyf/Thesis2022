...
def merge_new_config(config, new_config):
    if "_BASE_CONFIG_" in new_config:
        with Path(new_config["_BASE_CONFIG_"]).open() as base_cfg_file:
            yaml_config = yaml.safe_load(base_cfg_file)
...
def cfg_from_yaml_file(cfg_path: Path, config):
    with cfg_path.open() as cfg_file:
        new_config = yaml.safe_load(cfg_file)
