...
__device__ inline void lidar_to_local_coords(float shift_x, float shift_y, float rot_angle,
...
__device__ inline int check_pt_in_box3d(const float *pt, const float *box3d, float &local_x,
                                        float &local_y) {
  // param pt: (x, y, z)
...
__global__ void generate_pts_mask_for_box3d(int boxes_num, int pts_num, int out_x, int out_y,
  ...
  // params pts_mask: (N, npoints): -1 means point doesnot in this box, otherwise: encode (x_idxs,
...
__global__ void collect_inside_pts_for_box3d(int boxes_num, int pts_num, int max_pts_each_voxel,
                                             int out_x, int out_y, int out_z, const int *pts_mask,
  ...
  for (int k = 0; k < pts_num; k++) {
    if (pts_mask[box_idx * pts_num + k] != -1) {
      ...
      unsigned int base_offset = x_idx * out_y * out_z * max_pts_each_voxel +
                                 y_idx * out_z * max_pts_each_voxel + z_idx * max_pts_each_voxel;
...
__global__ void roiaware_maxpool3d(int boxes_num, int pts_num, int channels, int max_pts_each_voxel,
                                   int out_x, int out_y, int out_z, const float *pts_feature,
                                   const int *pts_idx_of_voxels, float *pooled_features,
                                   int *argmax) {
  ...
  if (box_idx >= boxes_num || channel_idx >= channels || x_idx >= out_x || y_idx >= out_y ||
      z_idx >= out_z)
    return;
  ...
  pts_idx_of_voxels +=
      box_idx * out_x * out_y * out_z * max_pts_each_voxel + offset_base * max_pts_each_voxel;
  pooled_features +=
      box_idx * out_x * out_y * out_z * channels + offset_base * channels + channel_idx;
...
#ifdef DEBUG
  printf(
      "channel_%d idx(%d, %d, %d), argmax_idx=(%d, %.3f), total=%d, after pts_idx: %p, argmax: "
      "(%p, %d)\n",
      channel_idx, x_idx, y_idx, z_idx, argmax_idx, max_val, total_pts, pts_idx_of_voxels, argmax,
      argmax_idx);
#endif
}

__global__ void roiaware_avgpool3d(int boxes_num, int pts_num, int channels, int max_pts_each_voxel,
                                   int out_x, int out_y, int out_z, const float *pts_feature,
                                   const int *pts_idx_of_voxels, float *pooled_features) {
  // params pts_feature: (npoints, C)
  // params pts_idx_of_voxels: (N, out_x, out_y, out_z, max_pts_each_voxel), index 0 is the counter
...
void roiaware_pool3d_launcher(int boxes_num, int pts_num, int channels, int max_pts_each_voxel,
                              int out_x, int out_y, int out_z, const float *rois, const float *pts,
                              const float *pts_feature, int *argmax, int *pts_idx_of_voxels,
                              float *pooled_features, int pool_method) {
  ...
  generate_pts_mask_for_box3d<<<blocks_mask, threads>>>(boxes_num, pts_num, out_x, out_y, out_z,
                                                        rois, pts, pts_mask);
  ...
  collect_inside_pts_for_box3d<<<blocks_collect, threads>>>(
      boxes_num, pts_num, max_pts_each_voxel, out_x, out_y, out_z, pts_mask, pts_idx_of_voxels);

  dim3 blocks_pool(DIVUP(out_x * out_y * out_z, THREADS_PER_BLOCK), channels, boxes_num);
  if (pool_method == 0) {
    roiaware_maxpool3d<<<blocks_pool, threads>>>(boxes_num, pts_num, channels, max_pts_each_voxel,
                                                 out_x, out_y, out_z, pts_feature,
                                                 pts_idx_of_voxels, pooled_features, argmax);
  } else if (pool_method == 1) {
    roiaware_avgpool3d<<<blocks_pool, threads>>>(boxes_num, pts_num, channels, max_pts_each_voxel,
                                                 out_x, out_y, out_z, pts_feature,
...
__global__ void roiaware_maxpool3d_backward(int boxes_num, int channels, int out_x, int out_y,
                                            int out_z, const int *argmax, const float *grad_out,
                                            float *grad_in) {
  // params argmax: (N, out_x, out_y, out_z, C)
...
__global__ void roiaware_avgpool3d_backward(int boxes_num, int channels, int out_x, int out_y,
                                            int out_z, int max_pts_each_voxel,
                                            const int *pts_idx_of_voxels, const float *grad_out,
                                            float *grad_in) {
  // params pts_idx_of_voxels: (N, out_x, out_y, out_z, max_pts_each_voxel)
  // params grad_out: (N, out_x, out_y, out_z, C)
  // params grad_in: (npoints, C), return value
...
void roiaware_pool3d_backward_launcher(int boxes_num, int out_x, int out_y, int out_z, int channels,
                                       int max_pts_each_voxel, const int *pts_idx_of_voxels,
  ...
  dim3 threads(THREADS_PER_BLOCK);
  if (pool_method == 0) {
    roiaware_maxpool3d_backward<<<blocks, threads>>>(boxes_num, channels, out_x, out_y, out_z,
                                                     argmax, grad_out, grad_in);
  } else if (pool_method == 1) {
    roiaware_avgpool3d_backward<<<blocks, threads>>>(boxes_num, channels, out_x, out_y, out_z,
                                                     max_pts_each_voxel, pts_idx_of_voxels,
                                                     grad_out, grad_in);
  }
}

__global__ void points_in_boxes_kernel(int batch_size, int boxes_num, int pts_num,
...
void points_in_boxes_launcher(int batch_size, int boxes_num, int pts_num, const float *boxes,
  ...
  dim3 threads(THREADS_PER_BLOCK);
  points_in_boxes_kernel<<<blocks, threads>>>(batch_size, boxes_num, pts_num, boxes, pts,
                                              box_idx_of_points);
