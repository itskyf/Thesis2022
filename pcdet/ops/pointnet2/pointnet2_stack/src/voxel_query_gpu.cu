...
__global__ void voxel_query_kernel_stack(int M, int R1, int R2, int R3, int nsample, float radius,
                                         int z_range, int y_range, int x_range,
                                         const float *new_xyz, const float *xyz,
                                         const int *new_coords, const int *point_indices,
                                         int *idx) {
  ...
  for (int dz = -1 * z_range; dz <= z_range; ++dz) {
    ...
    for (int dy = -1 * y_range; dy <= y_range; ++dy) {
      ...
      for (int dx = -1 * x_range; dx <= x_range; ++dx) {
        ...
        int index = batch_idx * R1 * R2 * R3 + z_coord * R2 * R3 + y_coord * R3 + x_coord;
        ...
        float dist2 = (x_per - new_x) * (x_per - new_x) + (y_per - new_y) * (y_per - new_y) +
...
void voxel_query_kernel_launcher_stack(int M, int R1, int R2, int R3, int nsample, float radius,
                                       int z_range, int y_range, int x_range, const float *new_xyz,
                                       const float *xyz, const int *new_coords,
  ...
  voxel_query_kernel_stack<<<blocks, threads>>>(M, R1, R2, R3, nsample, radius, z_range, y_range,
