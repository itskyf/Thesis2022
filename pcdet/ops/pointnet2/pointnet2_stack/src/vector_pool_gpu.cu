...
PV-RCNN++: Point-Voxel Feature Set Abstraction With Local Vector Representation for 3D Object
Detection https://arxiv.org/abs/2102.00463
...
__global__ void query_three_nn_by_stacked_local_idxs_kernel(
    ...
    int *new_xyz_grid_idxs, float *new_xyz_grid_dist2, const int *stack_neighbor_idxs,
  ...
  for (int k = 0; k < neighbor_length; k++) {
    ...
    float d = (center_x - x) * (center_x - x) + (center_y - y) * (center_y - y) +
...
__global__ void query_stacked_local_neighbor_idxs_kernel(
    ...
    const int *new_xyz_batch_cnt, int *stack_neighbor_idxs, int *start_len, int *cumsum,
    int avg_length_of_neighbor_idxs, float max_neighbour_distance, int batch_size, int M,
    int nsample, int neighbor_type) {
  // support_xyz: (N1 + N2 ..., 3) xyz coordinates of the features
  ...
  for (int k = 0; k < n; ++k) {
    ...
    if (neighbor_type == 1) {
      ...
      // voxel
      if ((fabs(local_x) > max_neighbour_distance) | (fabs(local_y) > max_neighbour_distance) |
...
int query_stacked_local_neighbor_idxs_kernel_launcher_stack(
  ...
  query_stacked_local_neighbor_idxs_kernel<<<blocks, threads>>>(
      support_xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, stack_neighbor_idxs, start_len,
      cumsum, avg_length_of_neighbor_idxs, max_neighbour_distance, batch_size, M, nsample,
      neighbor_type);
...
__global__ void vector_pool_kernel_stack(
    ...
    int num_grid_x, int num_grid_y, int num_grid_z, float max_neighbour_distance, int batch_size,
    int M, int num_c_in, int num_c_out, int num_c_each_grid, int num_total_grids,
    int *point_cnt_of_grid, int *grouped_idxs, int use_xyz, float grid_size_x, float grid_size_y,
    float grid_size_z, int *cum_sum, int num_max_sum_points, int nsample, int neighbor_type,
    int pooling_type) {
  // support_xyz: (N1 + N2 ..., 3) xyz coordinates of the features
  ...
  // grouped_idxs: (num_max_sum_points, 3)[idx of support_xyz, idx of new_xyz, idx of grid_idx in
  // new_xyz] use_xyz: whether to calculate new_local_xyz neighbor_type: 1: ball, others: cube
  ...
  for (int k = 0; k < n; ++k) {
    ...
    if (pooling_type == 0) {
      ...
      for (int i = 0; i < num_c_in; i++) {
        new_features[grid_idx * num_c_each_grid + i % num_c_each_grid] +=
            support_features[k * num_c_in + i];
      }
      if (use_xyz) {
      ...
      // printf("new_xyz=(%.2f, %.2f, %.2f, ), find neighbor k=%d: support_xyz=(%.2f, %.2f, %.2f),
      // local_xyz=(%.2f, %.2f, %.2f), neighbor=%.2f, grid_idx=%d, point_cnt_of_grid_idx=%d\n",
      // new_x, new_y, new_z, k, support_xyz[k * 3 + 0], support_xyz[k * 3 + 1], support_xyz[k * 3 +
      // 2], local_x, local_y, local_z, max_neighbour_distance, grid_idx,
      // point_cnt_of_grid[grid_idx]);
...
int vector_pool_kernel_launcher_stack(const float *support_xyz, const float *support_features,
                                      const int *xyz_batch_cnt, const float *new_xyz,
                                      float *new_features, float *new_local_xyz,
                                      const int *new_xyz_batch_cnt, int *point_cnt_of_grid,
                                      int *grouped_idxs, int num_grid_x, int num_grid_y,
                                      int num_grid_z, float max_neighbour_distance, int batch_size,
                                      int N, int M, int num_c_in, int num_c_out,
                                      int num_total_grids, int use_xyz, int num_max_sum_points,
                                      int nsample, int neighbor_type, int pooling_type) {
  // support_xyz: (N1 + N2 ..., 3) xyz coordinates of the features
  // support_features: (N1 + N2 ..., C)
  // xyz_batch_cnt: (batch_size), [N1, N2, ...]
  // new_xyz: (M1 + M2 ..., 3) centers of the ball query
  ...
  // grouped_idxs: (num_max_sum_points, 3)[idx of support_xyz, idx of new_xyz, idx of grid_idx in
  // new_xyz] neighbor_type: 1: ball, others: cube pooling_type: 0: avg_pool, 1: random choice
  ...
  vector_pool_kernel_stack<<<blocks, threads>>>(
      support_xyz, support_features, xyz_batch_cnt, new_xyz, new_features, new_local_xyz,
      new_xyz_batch_cnt, num_grid_x, num_grid_y, num_grid_z, max_neighbour_distance, batch_size, M,
      num_c_in, num_c_out, num_c_each_grid, num_total_grids, point_cnt_of_grid, grouped_idxs,
      use_xyz, grid_size_x, grid_size_y, grid_size_z, p_cum_sum, num_max_sum_points, nsample,
...
__global__ void vector_pool_grad_kernel_stack(const float *grad_new_features,
                                              ...
                                              float *grad_support_features, int N, int M,
                                              int num_c_out, int num_c_in, int num_c_each_grid,
                                              int num_total_grids, int num_max_sum_points) {
  // grad_new_features: (M1 + M2 ..., C_out)
  // point_cnt_of_grid: (M1 + M2 ..., num_total_grids)
  // grouped_idxs: (num_max_sum_points, 3) [idx of support_xyz, idx of new_xyz, idx of grid_idx in
  // new_xyz] grad_support_features: (N1 + N2 ..., C_in)
...
void vector_pool_grad_kernel_launcher_stack(const float *grad_new_features,
                                            const int *point_cnt_of_grid, const int *grouped_idxs,
                                            float *grad_support_features, int N, int M,
                                            int num_c_out, int num_c_in, int num_total_grids,
  ...
  vector_pool_grad_kernel_stack<<<blocks, threads>>>(
      grad_new_features, point_cnt_of_grid, grouped_idxs, grad_support_features, N, M, num_c_out,
      num_c_in, num_c_each_grid, num_total_grids, num_max_sum_points);
  // cudaDeviceSynchronize();  // for using printf in kernel function
  err = cudaGetLastError();
  if (cudaSuccess != err) {
    fprintf(stderr, "CUDA kernel failed : %s\n", cudaGetErrorString(err));
