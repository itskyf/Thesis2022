/*
Stacked-batch-data version of point interpolation, modified from the original implementation of
official PointNet++ codes. Written by Shaoshuai Shi All Rights Reserved 2019-2020.
*/
...
__global__ void three_nn_kernel_stack(int batch_size, int N, int M, const float *unknown,
                                      const int *unknown_batch_cnt, const float *known,
                                      const int *known_batch_cnt, float *dist2, int *idx) {
  // unknown: (N1 + N2 ..., 3)
...
void three_nn_kernel_launcher_stack(int batch_size, int N, int M, const float *unknown,
  ...
  three_nn_kernel_stack<<<blocks, threads>>>(batch_size, N, M, unknown, unknown_batch_cnt, known,
...
void three_interpolate_kernel_launcher_stack(int N, int channels, const float *features,
                                             const int *idx, const float *weight, float *out) {
  // features: (M1 + M2 ..., C)
  // idx: [N1 + N2 ..., 3]
  // weight: [N1 + N2 ..., 3]
...
void three_interpolate_grad_kernel_launcher_stack(int N, int channels, const float *grad_out,
  ...
  dim3 threads(THREADS_PER_BLOCK);
  three_interpolate_grad_kernel_stack<<<blocks, threads>>>(N, channels, grad_out, idx, weight,
                                                           grad_features);
